  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(300px, 1fr));gap:24px;">
        ...
        ${todayOrders.length === 0 ? `<p style="color:var(--text-muted);">${t('noOrders', 'admin')}</p>` :
          todayOrders.slice(0,5).map(o => `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border);">
              <div>
                <strong>${t('tables', 'admin')} ${o.tableNo || '?'}</strong>
                <span style="color:var(--text-muted);font-size:0.8rem;margin-left:8px;">${o.items?.length || 0} ${t('items', 'admin')}</span>
              </div>
      ...
      <div class="card">
        ...
        ${activeCallsCount === 0 ? `<p style="color:var(--text-muted);">${t('noCalls', 'admin')}</p>` :
          calls.filter(c => c.status === 'active').slice(0,5).map(c => `
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);">
              ...
              <div style="flex:1;">
                <strong>${t('tables', 'admin')} ${c.tableNo || '?'}</strong>
                <p style="font-size:0.8rem;color:var(--text-muted);">${t('waiterCalling', 'admin')}</p>
              </div>
            </div>
          `).join('')}
      </div>
